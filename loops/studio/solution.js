...
for (let i=0; i < numMeals; i++) {
  for (let j=0; j<5; j++) {
    mealArray.push(pantry[j][i])
  }
  meals.push(mealArray);
  mealArray=[];
}
// meals.push(mealArray.slice(0,5), mealArray.slice(5,10), mealArray.slice(10,15), mealArray.slice(15,20), mealArray.slice(20,25), mealArray.slice(25,30));
...
while (numMeals < 1 || numMeals > 6 || isNaN(numMeals)) {
  numMeals = Number(input.question("Invalid input. Enter a number between 1 and 6: "))
}
